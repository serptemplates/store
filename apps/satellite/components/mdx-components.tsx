...
export const mdxComponents: MDXComponents = {
  ...
  h2: ({ children }) => <h2 className="mb-4 mt-8 text-3xl font-semibold tracking-tight">{children}</h2>,
  ...
  blockquote: ({ children }) => (
    <blockquote className="my-6 border-l-4 border-primary/60 pl-6 italic text-muted-foreground">{children}</blockquote>
  ),
  ...
  img: ({ src, alt }) => (
    <Image src={src as string} alt={(alt as string) ?? ""} width={960} height={540} className="my-8 rounded-lg" />
  ),
