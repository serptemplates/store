...
- Validate your content with `pnpm --filter @apps/store validate:content` (now part of `pnpm --filter @apps/store lint`). The validator normalises null/empty values, enforces URL host allow-lists, and ensures rating fields stay within bounds.
