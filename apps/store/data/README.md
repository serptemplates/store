...
- The canonical schema is `product.schema.json`, generated from the Zod definition in `@/lib/products/product-schema.ts`. Marketing surfaces (copy, media, FAQs, reviews) and operational metadata (Stripe, GoHighLevel, licensing) are documented there so every consumer shares the same contract.
...
- **Marketing surfaces** – `features`, `benefits`, `pricing`, `screenshots`, `product_videos`, `related_videos`, `related_posts`, `faqs`, `reviews`, and optional badges (`featured`, `new_release`, `popular`). Currency codes are auto-uppercased and constrained to ISO 4217; review ratings must sit between 0–5.
...
- `checkout_metadata` is now reserved for runtime flags that are read directly from product config. The converter keeps only `bundle_expand` / `bundle_expand_mode` (used by bundle entitlement expansion). Everything else is dropped during `convert:products`.
- Stripe Checkout forwards a small allowlist to keep the Stripe metadata payload lean: `product_slug`, `ghl_tag`, `affiliateId`, `dubCustomerExternalId`, and `dubClickId`. If a key must reach Stripe metadata, add it to the allowlist in `apps/store/app/checkout/[slug]/route.ts`.
- Do not add new metadata here unless the runtime reads it directly. For CRM/analytics data, prefer explicit product fields or provider-specific metadata (`payment.*.metadata`) so we do not rely on Stripe metadata.
