...
import logger from "@repo/payments/logger";
...
type StripeDeps = {
  ...
  resolvePriceForEnvironment: typeof resolvePriceForEnvironment;
};

let stripeDeps: StripeDeps = {
  getStripeClient,
  resolvePriceForEnvironment,
};

export function setStripeCheckoutDependencies(overrides: Partial<StripeDeps>) {
  stripeDeps = {
    ...stripeDeps,
    ...overrides,
  };
...
export function resetStripeCheckoutDependencies() {
  stripeDeps = {
    getStripeClient,
    resolvePriceForEnvironment,
  };
...
async function buildOptionalItems(
  ...
  for (const optionalItem of items) {
    try {
      const liveStripe = stripeClient({ mode: "live", accountAlias: context.accountAlias ?? undefined });
      ...
      const optionalPrice = await priceResolver(
        {
...
export const stripeCheckoutAdapter: PaymentProviderAdapter = {
  ...
  async createCheckout(request: CheckoutRequest): Promise<CheckoutResponse> {
    ...
    const resolvedPrice = await priceResolver(
      {
        id: request.slug,
    ...
    const stripe = stripeClient({ accountAlias: request.paymentAccountAlias ?? undefined });
