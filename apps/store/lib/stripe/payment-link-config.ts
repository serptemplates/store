...
function isConsentParameterUnknown(error: Stripe.errors.StripeError | undefined): boolean {
  if (!error) {
    return false;
  }
  ...
  return typeof error.message === "string" && error.message.includes("consent_collection");
}
