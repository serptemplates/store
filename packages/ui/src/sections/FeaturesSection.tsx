      ...
      <div className="mx-auto max-w-6xl">
        <div className="rounded-3xl  p-6  md:p-10">
          <div className="mb-8 pb-6 text-center">
            <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Features</h2>
          </div>
          <ul className="grid grid-cols-1 gap-3 lg:grid-cols-2 xl:grid-cols-3">
            {features.map((feature, index) => (
              <li key={`feature-${index}`} className="flex flex-col items-start gap-2">
                <span className="flex items-center gap-1 text-sm font-medium text-gray-900 leading-6">
                ...
                {feature.description ? (
                  <span className="pl-10 text-sm text-gray-500 leading-6">{feature.description}</span>
                ) : null}
              </li>
            ))}
          </ul>
