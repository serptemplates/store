...
- Product JSON now declares `pricing.cta_href` values that always point to `/checkout/<slug>` (absolute URLs fine for schema validation; runtime normalizes them to relative paths).
...
- `useProductCheckoutCta` intercepts clicks, calls `/api/checkout/session` via `createSessionWithDub`, and falls back to the same checkout URL if the programmatic call fails. Waitlist CTAs still display the modal for `status === "pre_release"`.
...
1. Prefer the existing `/api/checkout/session` workflow for new Stripe surfaces. Only branch into provider-specific flows when a PSP cannot be modelled as a Stripe Checkout Session.
...
3. Extend Vitest coverage for any new helpers under `apps/store/tests/**`, and keep E2E coverage in Playwright focused on CTA navigation + session creation rather than Payment Link redirects.
